...
public class MatchingServices {
    ...
    public List<Found_items> findMatchesForLost(Lost_items lost) {
        ...
        for (Found_items found : matches) {
            notificationService.sendWhatsAppNotification(lost.getContact(),
                    "📢 Match found for your lost item '" + lost.getItemName() +
                            "' at location: " + found.getLocation() +
                            "\n👤 Finder’s contact: " + found.getContact());

            notificationService.sendWhatsAppNotification(found.getContact(),
                    "🎉 Your found item '" + found.getItemName() +
                            "' matches a lost item at location: " + lost.getLocation() +
                            "\n👤 Loser’s contact: " + lost.getContact());

            Match_history match = new Match_history();
            match.setLostItem(lost);
            match.setFoundItem(found);
            match.setLostContact(lost.getContact());
            match.setFoundContact(found.getContact());
